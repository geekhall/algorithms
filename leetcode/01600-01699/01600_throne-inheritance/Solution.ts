...
class SuccessorNode {
  name: string
  father: SuccessorNode | null
  is_dead: boolean
  children: Array<SuccessorNode>
...
class ThroneInheritance {
  root: SuccessorNode
  nodeMap: Map<string, SuccessorNode>
  constructor(kingName: string) {
    this.root = new SuccessorNode(kingName)
    this.nodeMap = new Map()
    this.nodeMap.set(kingName, this.root)
  }

  birth(parentName: string, childName: string): void {
    let childNode = new SuccessorNode(childName)
    this.nodeMap.get(parentName)!.children.push(childNode)
    this.nodeMap.set(childName, childNode)
  }

  death(name: string): void {
    this.nodeMap.get(name)!.is_dead = true
  }

  getInheritanceOrder(): string[] {
    let order = new Array()
    const dfs = (node: SuccessorNode | null) => {
      if (!node)
        return
