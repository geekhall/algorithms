 ...
 * Difficulty: Easy
 * Description: You are assigned to put some amount of boxes onto one truck. You are given a 2D array boxTypes, where boxTypes[i] = [numberOfBoxes i, numberOfUnitsPerBox i ]:
 *
...
function maximumUnits(boxTypes: number[][], truckSize: number): number {
  ...
  for (let i = 0; i < boxTypes.length; i++) {
    if (boxTypes[i][0] + sum <= truckSize) {
      sum += boxTypes[i][0]
      maxUnits += boxTypes[i][1] * boxTypes[i][0]
    } else {
      while (boxTypes[i][0] + sum > truckSize) {
        boxTypes[i][0]--
      }
