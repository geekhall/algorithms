...
function countStudents(students: number[], sandwiches: number[]): number {
  ...
  while (currentSandwich < sandwiches.length) {
    if (students[currentStudent] === sandwiches[currentSandwich]) {
      count[sandwiches[currentSandwich]]--
      currentSandwich++
    }
    ...
    if (count[0] === 0 && count[1] === 0) {
      res = 0
      break
    } else if (count[0] === 0 && sandwiches[currentSandwich] === 0) {
      res = count[1]
      break
    } else if (count[1] === 0 && sandwiches[currentSandwich] === 1) {
      res = count[0]
      break
    }
  }
