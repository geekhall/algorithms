...
function pathSum(root: TreeNode | null, targetSum: number): number[][] {
  ...
  if (root.left) {
    result.push(...pathSum(root.left, targetSum - root.val).map(path => [root.val, ...path]))
  }
  if (root.right) {
    result.push(...pathSum(root.right, targetSum - root.val).map(path => [root.val, ...path]))
  }
  return result
};

function test_00113() {
  let root = TreeNode.create([5, 4, 8, 11, null, 13, 4, 7, 2, null, null, 5, 1]), targetSum = 22
  console.log(pathSum(root, targetSum))
  root = TreeNode.create([1, 2, 3]), targetSum = 5
  console.log(pathSum(root, targetSum))
}
