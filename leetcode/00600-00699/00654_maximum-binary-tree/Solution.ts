...
function constructMaximumBinaryTree(nums: number[]): TreeNode | null {
  ...
  let res: TreeNode | null = new TreeNode(nums[max], null, null)
  res.left = constructMaximumBinaryTree(nums.slice(0, max))
  res.right = constructMaximumBinaryTree(nums.slice(max + 1))
  return res
};
