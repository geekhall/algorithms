...
function lcaDeepestLeaves(root: TreeNode | null): TreeNode | null {
  ...
  const dfs = (node: TreeNode | null, depth: number): ResultType => {
    if (!node) {
    ...
    if (left.depth > right.depth) {
      return { node: left.node, depth: left.depth }
    } else if (left.depth < right.depth) {
      return { node: right.node, depth: right.depth }
    } else {
...
function test_01123() {
  ...
  let res = lcaDeepestLeaves(root)
  TreeNode.print(res)
  root = TreeNode.create([1])
  res = lcaDeepestLeaves(root)
  TreeNode.print(res)
  root = TreeNode.create([0, 1, 3, null, 2])
  res = lcaDeepestLeaves(root)
  TreeNode.print(res)
}
