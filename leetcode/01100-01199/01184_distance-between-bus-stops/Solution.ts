...
function distanceBetweenBusStops(distance: number[], start: number, destination: number): number {
  let clockwise = 0
  let all = distance.reduce((a, b) => a + b)

  if (start > destination) {
    clockwise = distance.slice(destination, start).reduce((a, b) => a + b)
  } else {
    clockwise = distance.slice(start, destination).reduce((a, b) => a + b)
  }

  return Math.min(clockwise, all - clockwise)
};
