...
function flipMatchVoyage(root: TreeNode | null, voyage: number[]): number[] {
  ...
  let i = 0
  const dfs = (node: TreeNode | null, voyage: number[]): boolean => {
    if (!node)
      return true
...
function test_00971() {
  let root = TreeNode.create([1, 2]), voyage = [2, 1]
  console.log(flipMatchVoyage(root, voyage))
  root = TreeNode.create([1, 2, 3]), voyage = [1, 3, 2]
  console.log(flipMatchVoyage(root, voyage))
  root = TreeNode.create([1, 2, 3]), voyage = [1, 2, 3]
  console.log(flipMatchVoyage(root, voyage))
