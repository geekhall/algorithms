 ...
 * Difficulty: Medium
 * Description: There is an m x n grid, where (0, 0) is the top-left cell and (m - 1, n - 1) is the bottom-right cell. You are given an integer array startPos where startPos = [start row, start col ] indicates that initially, a robot is at the cell (start row, start col). You are also given an integer array homePos where homePos = [home row, home col ] indicates that its home is at the cell (home row, home col).
 *
 ...
 * startPos.length == 2
 * homePos.length == 2
 * 0 <= start row, home row < m
 * 0 <= start col, home col < n
 */
function minCost(startPos: number[], homePos: number[], rowCosts: number[], colCosts: number[]): number {
  ...
  while (i !== x) {
    i += (x - i) / Math.abs(x - i)
    min_cost += rowCosts[Math.floor(i)]
  }
  while (j !== y) {
    j += (y - j) / Math.abs(y - j)
    min_cost += colCosts[Math.floor(j)]
  }
  return min_cost
};
...
function test_02087() {
  ...
  startPos = [5, 5], homePos = [5, 2], rowCosts = [7, 1, 3, 3, 5, 3, 22, 10, 23], colCosts = [5, 5, 6, 2, 0, 16]
  console.log(minCost(startPos, homePos, rowCosts, colCosts))
