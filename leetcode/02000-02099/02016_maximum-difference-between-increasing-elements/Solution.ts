...
function maximumDifference1(nums: number[]): number {
  ...
  let max = -1
  for (let i = 0; i < n - 1; i++) {
    let max_i = Math.max(...nums.slice(i + 1))
    if (max_i > nums[i] && max_i - nums[i] > max)
      max = max_i - nums[i]
  }
  return max
};
function maximumDifference(nums: number[]): number {
  ...
  let i = nums.length - 2;
  let temp_max = nums[nums.length - 1]
  while (i >= 0) {
    if (nums[i] >= temp_max) {
      temp_max = nums[i];
    }
    else {
      max_diff = Math.max(max_diff, temp_max - nums[i]);
    }
    i--;
  }
...
function test_02016() {
  ...
  console.log(maximumDifference(nums)); // expected -1
