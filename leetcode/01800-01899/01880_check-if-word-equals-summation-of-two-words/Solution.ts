...
function isSumEqual(firstWord: string, secondWord: string, targetWord: string): boolean {
  let firstValue = parseInt(Array.from(firstWord).map((v, i) => {
    return (v.charCodeAt(0) - 97).toString()
  }).join(''))
  let secondValue = parseInt(Array.from(secondWord).map((v, i) => {
    return (v.charCodeAt(0) - 97).toString()
  }).join(''))
  let targetValue = parseInt(Array.from(targetWord).map((v, i) => {
    return (v.charCodeAt(0) - 97).toString()
  }).join(''))
