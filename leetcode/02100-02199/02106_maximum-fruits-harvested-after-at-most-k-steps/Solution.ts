 ...
 * Difficulty: Hard
 * Description: Fruits are available at some positions on an infinite x-axis. You are given a 2D integer array fruits where fruits[i] = [position i, amount i ] depicts amount i fruits at the position position i. fruits is already sorted by position i in ascending order, and each position i is unique.
 * 
 ...
 * fruits[i].length == 2
 * 0 <= startPos, position i <= 2 * 10 5
 * position i-1 < position i for any i > 0 (0-indexed)
 * 1 <= amount i <= 10 4
 * 0 <= k <= 2 * 10 5
 */
function solution() {
