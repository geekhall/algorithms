 ...
 * Example 1:
 *
 * Input: words = ["a","b","ba","bca","bda","bdca"] Output: 4 Explanation: One of the longest word chains is ["a"," b a","b d a","bd c a"].
 *
 * Example 2:
 *
 * Input: words = ["xbc","pcxbcf","xb","cxbc","pcxbc"] Output: 5 Explanation: All the words can be put in a word chain ["xb", "xb c", " c xbc", " p cxbc", "pcxbc f"].
 *
 * Example 3:
 *
...
function longestStrChain(words: string[]): number {
  ...
  for (let word of words) {
    for (let i = 0; i < word.length; i++) {
      ...
      let prefixWord = prefix + suffix
      if (map.has(prefixWord)) {
        map.set(word, Math.max(map.get(word)!, map.get(prefixWord)! + 1))
      }
    }
  }
  let max = 0
