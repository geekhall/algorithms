 ...
 */
// heap out of memory
function gridIllumination1(n: number, lamps: number[][], queries: number[][]): number[] {
  let res = new Array();
  // 0 - off, 1 - on, 2 - lamp
  let lampsMap = new Array(n).fill(0).map(() => new Array(n).fill(0));
  const hasLamp = (row: number, col: number): boolean => {
    let res = false
    if (lampsMap[row][col] === 2) {
      return true
    }
    for (let i = 0; i < n; i++) {
      if (lampsMap[i][col] === 2) {
      ...
      if ((row - i >= 0 && row - i < n && col - i >= 0 && col - i < n && lampsMap[row - i][col - i] === 2)
        || (row - i >= 0 && row - i < n && col + i >= 0 && col + i < n && lampsMap[row - i][col + i] === 2)
        || (row + i >= 0 && row + i < n && col - i >= 0 && col - i < n && lampsMap[row + i][col - i] === 2)
        || (row + i >= 0 && row + i < n && col + i >= 0 && col + i < n && lampsMap[row + i][col + i] === 2)) {
        res = true
        break
  ...
  const turn = (row: number, col: number, onFlag: boolean) => {
    if (onFlag) {
  ...
  const lampOn = (row: number, col: number) => {
    lampsMap[row][col] = 2;
    for (let i = 0; i < n; i++) {
      if (lampsMap[row][i] === 0)
        lampsMap[row][i] = 1
    ...
    for (let i = 1; i < n; i++) {
      turn(row - i, col - i, true);
      turn(row - i, col + i, true);
      turn(row + i, col - i, true);
      turn(row + i, col + i, true);
    }
  }
  const lampOff = (row: number, col: number) => {
    if (hasLamp(row, col)) {
      lampsMap[row][col] = 1;
    } else {
      lampsMap[row][col] = 0;
    ...
    for (let i = 0; i < n; i++) {
      if (lampsMap[row][i] === 1 && !hasLamp(row, i))
        lampsMap[row][i] = 0
      if (lampsMap[i][col] === 1 && !hasLamp(i, col))
        lampsMap[i][col] = 0;
    }
    for (let i = 1; i < n; i++) {
      turn(row - i, col - i, false);
      turn(row - i, col + i, false);
      turn(row + i, col - i, false);
      turn(row + i, col + i, false);
    }
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    res.push(lampsMap[row][col] === 1 || lampsMap[row][col] === 2 ? 1 : 0);

    for (let j = 0; j < directions.length; j++) {
      let [dRow, dCol] = directions[j];
      let row2 = row + dRow;
      let col2 = col + dCol;
      if (row2 >= 0 && row2 < n && col2 >= 0 && col2 < n && lampsMap[row2][col2] === 2) {
        lampOff(row2, col2);
      }
    }
  }
